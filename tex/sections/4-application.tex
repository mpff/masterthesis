...
Taken together this shows that the proposed mean estimation method provide elastic mean estimates in the setting of sparse and irregular curves, which are invariant to all shape-preserving transformations.


\section{Elastic Full Procrustes Fits and Outliers}
\label{sec:4-pfits}
\todo[inline]{Überlegen wie ich diese section schreibe. Procrustes fits, outlier plots.}
\begin{figure}
  \centering
  ...
  \end{subfigure}
  \caption{Elastic full Procrustes fits (grey) and smooth mean (red). Data: See \cref{fig:41-digit3-means-b,fig:41-spiral-means-b}}
  \label{fig:4-pfits}
\end{figure}
\begin{itemize}
  \item Discuss elastic Procrustes fits.
  \item Discuss warping aligned Procrustes fits in last iteration
  \item Plot Procrustes fits from last iteration together with mean.
  \item Discuss outliers that get shrinked away $\rightarrow$ plot.
  \item Discuss normalization problem when keeping scaling fixed $\rightarrow$ Appendix?
\end{itemize}
\todo[inline]{Compare sum-of-squared distances to mean for three mean types. Can only compare the two Procrustes means}
\begin{figure}
  \centering
  \begin{subfigure}{0.48\textwidth}
  ...
  \end{subfigure}
  \caption{Elastic full Procrustes fits (grey) and piecewise-linear mean (red) on SRV level for digits (left) and spirals (right). Data: See \cref{fig:41-digit3-means-b,fig:41-spiral-means-b}}
...
The order two mean (right) is slightly longer towards the end, indicating a speed up, which is caused by the penalization towards a linearly (increasing) velocity, compared to a more conservative penalization towards a constant velocity.\todo{Warum sieht p=1 nicht polygon aus?} 
