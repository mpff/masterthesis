...
Following ...\todo{Cite!}, we can treat this estimation as a smoothing problem, by constructing responses \todo{Index $k$ für step ändern} $y^{(k)}_{ijk} = \widetilde q^{(k)}_i(s_{ij}) \overline{\widetilde q^{(k)}_i(s_{ik})}$ and treating the pairs $s_{ij}$, $s_{ik}$ as covariates $s$ and $t$.
...
\todo[inline]{Erklärung Penalty? Motivate b-spline/p-spline basis using Lisa's paper? REML?}
