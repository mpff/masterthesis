...
Note that this does not influence the normalization as warping is norm-preserving.
...
This still allows us the flexibility of estimating either polygonal or smooth means, as we estimate the mean function on original curve level by integration. 
...
Multiplying by $\theta^H G$ from the left and using $\theta^H G \theta = 1$ yields $\lambda = \theta^H G \hat\Xi G \theta$, i.e. the eigenvalues correspond to the target function to maximize.
...
\begin{figure}
  ...
  \caption{Elastic full Procrustes mean of handwritten digits \enquote*{3}.
    Estimated using 13 equidistant knots, a 2nd order penalty and piecewise constant (left) or piecewise linear (right) B-splines.
...
\label{sec:3-pfits}
Given an estimated mean function $\hat\mu_q^{(k)} = b(s)^\top\hat\theta$, we need to calculate the Procrustes alignment $\omega_i^{(k)} = \langle \widetilde q_i^{(k)}, \hat\mu_q^{(k)} \rangle$ of each curve onto the current mean, before we can update the warping alignment (see \cref{algo:mean}).
...
\end{equation}
Following \cite{Steyer2021} and as discussed in \cref{sec:3-discrete}, this may be achieved by treating $\widetilde q_i^{(k)}$ as piecewise constant with the normalized and warping aligned values given by
\begin{equation}
...
\langle \widetilde q_i^{(k)}, \hat\mu_q^{(k)} \rangle =
  \int_0^1 \langle \widetilde q_i^{(k)}(t), \hat\mu_q^{(k)}(t) \rangle \dt =
  \sum_{j=0}^{m-1} \int_{\left(\gamma_i^{(k)}\right)^{-1}(t_{ij})}^{\left(\gamma_i^{(k)}\right)^{-1}(t_{ij+1})} \langle \widetilde q_{ij}^{(k)},\, \hat\mu_q^{(k)} (t) \rangle \dt
\end{equation}
